    ...
    file_list.extend(
        {
            ...
            'target_hpp': os.path.join(
                source_base, fname.replace('.json', '.hpp')
            ),
            'target_cpp': os.path.join(
                target_base, fname.replace('.json', '.cpp')
            ),
        }
...
class EnumClass:
    def __init__(self, entry):
        ...
        self.values = []
        self.values.extend(
            EnumMember(value_entry, index)
            for index, value_entry in enumerate(entry['values'])
        )
...
for entry in file_list:
    ...
    with open(target_header, 'w+') as f:
        include_text = '#pragma once\n\n' + ''.join(
            [include_base.replace('${FILENAME}', x) for x in include_list]
        )
